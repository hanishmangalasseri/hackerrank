...
with an integer . Print a list of all possible coordinates given by  on a 3D grid where the sum of  is not equal to .
...
if __name__ == '__main__':
    ...
    z = int(input())
    n = int(input())
    sample_array_out = [[i, j, k] for i in range(x+1) for j in range(y+1) for k in range(z+1) if i+j+k != n]
print(sample_array_out)
